...
define("VERBOSE","0"); // mettre à 1 pour afficher les messages lors de l'installation en plus des logs
...
# DEFINITION DES FONCTIONS
  ...
  function _myextension_remove_wp_capabilities() { // arguments role, caps
    ...
    if ($role) {
      echo PHP_EOL.'SUPPRESSION de toutes les permissions pour le role '.$roleName.PHP_EOL;
      foreach ($caps as $cap) {
        //echo $cap." ";
        $role->remove_cap($cap);
        }
...
# IMPLEMENTS hook_civicrm_managed().
  ...
  function don_corps_civicrm_managed(&$entities) {
    ...
    $msg="  - Modification des parametres de localisation, date...".PHP_EOL; //LOG OK
      echo $msg;
      $fp=fopen(LOGFILE, 'a'); // ouvre le fichier de log
      fwrite($fp, $msg);
      fclose($fp);
