...
class CRM_DonCorps_CivirulesActions_Activite_Supprimelot extends CRM_Civirules_Action {
  ...
  public function processAction(CRM_Civirules_TriggerData_TriggerData $triggerData) {
    ...
    if(count($pieces_a_detruire)!=0){
        $rapport1="<p>####### Pièces détruites par cette action : ".implode(", ",$pieces_a_detruire)."</p>";
        CRM_Core_Session::setStatus(implode(", ",$pieces_a_detruire), 'Pièces détruites par cette action', 'success');
        } else{
        $rapport1="";
    }
    
    if(count($pieces_deja_detruites)!=0){
        $rapport2="<p>####### détruites préalablement à cete action --> non modifiées ".implode(", ",$pieces_deja_detruites)."</p>";
        CRM_Core_Session::setStatus(implode(", ",$pieces_deja_detruites), 'Pièces déjà détruites : inchangées', 'success');
        } else{
        $rapport2="";
    }
    
    if(count($pieces_corps)!=0){
        $rapport3="<p>####### Codes barres correspondant à des corps  --> inchangées : ".implode(", ",$pieces_corps)."</p>";
        CRM_Core_Session::setStatus(implode(", ",$pieces_corps), 'Codes barres correspondant à des corps : utiliser le tableau de bord et demander crémation', 'alert');
        } else{
        $rapport3="";
    }
    
    if(count($pieces_noUtilisation)!=0){
        $rapport4="<p>####### Pièces absentes de la base --> inchangées : ".implode(", ",$pieces_noUtilisation)."</p>";  
        CRM_Core_Session::setStatus(implode(", ",$pieces_noUtilisation), 'Pièces absentes de la base : inchangées', 'alert'); 
        } else{
        $rapport4="";
    }
