...
Tester l'éxécution de *job.execute* par l'utilisateur *www-data$ et l'utilisateur *cronuser* de Wordpress
...
Normalement les taches s’exécutent et sont listés dans les journaux accessibles dans :
**Administrer > Paramètres Systèmes > Travaux programmés**

## Régler cron
Cron permet de définir la périodicité d'exécution de job.execute.

Par exemple, pour lancer job.execute toutes les 5 minutes:
* éditer crontab
```crontab -e -u www-data``` <br>

* ajouter la ligne suivante dans le crontab de www-data <br>
