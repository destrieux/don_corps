...
* Créer la base civicrm <br>
```mysql -u root -p -e "CREATE DATABASE civicrm DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci"```

* Donner tous les droits à l'utilisateur civicrm ayant le mot de passe *MDPCIVICRM*<br>
...
* Déplacez et dezippez l'archive téléchargée dans les pluggins de Wordpress <br>
...
* Connectez vous sur l'interface d'administration du site Wordpress ```https://ddctest/wp-admin/``` avec des permissions admin. <br>
```identifiant : USERWORDPRESS``` <br>
```password : MDPUSERWORDPRESS``` <br>

* Allez dans les extensions de Wordpress et activez CiviCRM pour lancer l'installation
```Langue: francais``` <br>
...
L'installation peut parfois rendre Wordpress inacessible en raison d'une erreur d'installation.
